...
class CorpusFramework:
    ...
    def filter_uniform_templates(self) -> int:
        ...
        if not uniform:
            return 0
        dropped = [r for r in self._ledger if r["template_id"] in uniform]
        keep_ids = {r["example_id"] for r in self._ledger if r["template_id"] not in uniform}
        self._ledger = [r for r in self._ledger if r["template_id"] not in uniform]
        self._docs   = [d for d in self._docs if d["id"].split("__source_to")[0] in keep_ids
                        or d["id"] in keep_ids]
        for r in dropped:
            self._rejected.append({"desc": r["example_id"], "reason": "uniform_template",
                                   "template_id": r["template_id"]})
        return len(dropped)

    def reduce_diagnostic_skew(self, max_pct: float = 0.40, max_iters: int = 400) -> int:
        """Greedy removal of over-represented diagnostic codes, preserving template mixing."""
        ...
        def stays_mixed(drop_id: str) -> bool:
            rem = [r for r in self._ledger if r["example_id"] != drop_id]
            by: dict[str, set] = defaultdict(set)
            for r in rem:
                by[r["template_id"]].add(r["expected_outcome"])
        ...
        for _ in range(max_iters):
            ...
            for r in list(self._ledger):
                if r["expected_outcome"] == "fail" and code in r["expected_diagnostic_codes"].split("|"):
                    if stays_mixed(r["example_id"]):
                        eid = r["example_id"]
                        self._ledger.remove(r)
                        self._docs = [d for d in self._docs
                                      if d["id"] != eid and d["id"] != f"{eid}__source_to_ownership_diagnostic"]
                        self._rejected.append({"desc": eid, "reason": "skew_reduction", "code": code})
                        dropped += 1
                        removed = True
                        break
            if not removed:
